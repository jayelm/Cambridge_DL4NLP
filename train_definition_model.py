...
def read_data(data_path, vocab_size, phase="train"):
  ...
  """
    glosses, heads = [], []
    gloss_path = os.path.join(data_path, "%s.definitions.ids%s.gloss" %
                              (phase, vocab_size))
    head_path = os.path.join(data_path,
                             "%s.definitions.ids%s.head" % (phase, vocab_size))
        ...
        with tf.gfile.GFile(head_path, mode="r") as head_file:
            ...
            while gloss and head:
                ...
                if counter % 100000 == 0:
                    ...
                    sys.stdout.flush()
                gloss_ids = np.array(
                    [int(x) for x in gloss.split()], dtype=np.int32)
    ...
    with tf.device("/device:GPU:0"):
        ...
        with tf.variable_scope("embeddings"):
            if pretrained_input:
                ...
                embedding_matrix = tf.get_variable(
                    name="inp_emb", shape=[vocab_size, emb_size])
        ...
        if pretrained_target:
            ...
            core_out = tf.contrib.layers.fully_connected(
                core_out, out_size, activation_fn=tf.tanh)
...
def query_model(sess,
    ...
    while True:
        ...
        padded_ids = np.asarray(
            data_utils.pad_sequence(token_ids, max_seq_len))
        ...
        with tf.device("/cpu:0"):
            with tf.Session() as sess:
                (input_node, target_node, predictions, loss, vocab,
                 rev_vocab) = restore_model(
                     sess, FLAGS.save_dir, vocab_file, out_form="cosine")
